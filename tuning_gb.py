...
def do_modeling(max_depth, max_features, X_train, X_test, y_train, y_test):
    gb_classifier = GradientBoostingClassifier(max_depth = max_depth, max_features = max_features)
...
if __name__ == "__main__":
    ...
    plt.ylabel("accuracy")
    plt.title("Grid Search - Gradient Boosting")
    plt.legend()
    plt.grid()
    plt.show()
    plt.savefig('Grid Search - Gradient Boosting')  # Save the figure to a file
    plt.close()
